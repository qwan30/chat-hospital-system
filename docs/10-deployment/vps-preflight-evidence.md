...
| PENDING — operator evidence required | Migration revision recorded | `docker compose -f "<absolute-path-to-infra/docker-compose.yml>" run --rm --no-deps backend alembic current` | Database revision is recorded against the candidate SHA after migration | `<operator-recorded-value>` | `<YYYY-MM-DDThh:mm:ssZ>` | `<owner>` |
