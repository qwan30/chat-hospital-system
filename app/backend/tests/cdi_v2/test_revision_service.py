...
async def test_stale_draft_save_returns_conflict_without_revision(session_and_settings, seeded_document) -> None:
    ...
    first = await service.save_page(
        seeded_document.id,
        1,
        SavePageCommand(text="first", parent_revision_id=machine_id, lock_version=1, actor_id=doctor_id, edit_reason="fix"),
    )
    with pytest.raises(ConflictError):
        await service.save_page(
            seeded_document.id,
            1,
            SavePageCommand(text="stale", parent_revision_id=machine_id, lock_version=1, actor_id=records_id, edit_reason="fix"),
        )
...
async def test_production_editor_cannot_approve_own_submission(session_and_settings, submitted_set) -> None:
    session, _ = session_and_settings
    from hospital_ai.services.revisions import RevisionService, ApproveRevisionCommand
    with pytest.raises(ConflictError):
...
async def test_submit_reject_and_restore(session_and_settings, seeded_document) -> None:
    session, _ = session_and_settings
    from hospital_ai.services.revisions import RevisionService, SubmitCommand, RejectCommand, RestoreCommand
    service = RevisionService(session)
    
    sub = await service.submit(seeded_document.id, SubmitCommand(actor_id=seeded_document.doctor_id))
    assert sub.status == "submitted"
    
    rejected = await service.reject(sub.revision_set_id, RejectCommand(actor_id=seeded_document.doctor_id, reason="incorrect"))
    assert rejected.status == "rejected"
    
    restored = await service.restore(seeded_document.id, 1, RestoreCommand(revision_id=seeded_document.machine_id, actor_id=seeded_document.doctor_id, lock_version=1, reason="undo"))
