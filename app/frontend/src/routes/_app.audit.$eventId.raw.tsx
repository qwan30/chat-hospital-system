...
function Page() {
  ...
  const sample = {
    ...
    result: "ok",
    traceId: "t-9f3a",
    policy: { allow: true, rules: ["role:cardiologist", "unit:4N"] },
  };
  return (
    <AppShell>
      <PageHeader
        title={`Audit event ${eventId}`}
        description="Raw JSON record (signed, append-only)."
      />
      <Card className="p-5">
        <pre className="overflow-auto text-xs">{JSON.stringify(sample, null, 2)}</pre>
      </Card>
    </AppShell>
  );
}
