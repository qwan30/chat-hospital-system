...
from hospital_ai.db.clinical_documents import DocumentRevisionSet, DocumentIndexGeneration
...
async def graph_fixture(session_and_settings):
    ...
    doc = Document(id=uuid.uuid4(), patient_id=patient_id, title="Doc", uploaded_by=DOCTOR_ID, mime_type="text/plain", storage_uri="mem", status="ready", document_type="clinical_note")
    session.add(doc)
    await session.flush()
