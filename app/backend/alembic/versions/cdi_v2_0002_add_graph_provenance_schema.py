...
def upgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['generation_id'], ['document_index_generations.id'], ),
    sa.ForeignKeyConstraint(['page_revision_id'], ['document_page_revisions.id'], ),
    sa.ForeignKeyConstraint(['patient_id'], ['patients.id'], ),
    sa.ForeignKeyConstraint(['revision_set_id'], ['document_revision_sets.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_graph_mentions_patient_id'), 'graph_mentions', ['patient_id'], unique=False)

    # Create graph_relation_assertions
    op.create_table('graph_relation_assertions',
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('patient_id', sa.Uuid(), nullable=False),
    ...
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.ForeignKeyConstraint(['object_entity_id'], ['graph_entities.id'], ),
    sa.ForeignKeyConstraint(['patient_id'], ['patients.id'], ),
    sa.ForeignKeyConstraint(['subject_entity_id'], ['graph_entities.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('patient_id', 'subject_entity_id', 'object_entity_id', 'relation_type', 'normalized_value', name='uq_graph_relation_assertion')
    )
    op.create_index(op.f('ix_graph_relation_assertions_patient_id'), 'graph_relation_assertions', ['patient_id'], unique=False)

    # Create graph_relation_evidence
    op.create_table('graph_relation_evidence',
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('patient_id', sa.Uuid(), nullable=False),
    ...
    sa.Column('chunk_id', sa.Uuid(), nullable=False),
    sa.Column('independent_source_identity', sa.String(length=128), nullable=False),
    sa.ForeignKeyConstraint(['assertion_id'], ['graph_relation_assertions.id'], ),
    sa.ForeignKeyConstraint(['chunk_id'], ['document_chunks.id'], ),
    sa.ForeignKeyConstraint(['document_id'], ['documents.id'], ),
    sa.ForeignKeyConstraint(['generation_id'], ['document_index_generations.id'], ),
    sa.ForeignKeyConstraint(['page_revision_id'], ['document_page_revisions.id'], ),
    ...
    op.create_index(op.f('ix_graph_relation_evidence_patient_id'), 'graph_relation_evidence', ['patient_id'], unique=False)
