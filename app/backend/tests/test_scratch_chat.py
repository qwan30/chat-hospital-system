...
async def test_10_chat_scenarios(session_and_settings):
    ...
    clinical_note = await create_indexed_document(
        session, patient_id=PATIENT_ALICE_ID, uploaded_by=DOCTOR_ID,
        title="Clinical Note", content="Patient is recovering well from surgery."
    )
    
    lab_result = await create_indexed_document(
        session, patient_id=PATIENT_ALICE_ID, uploaded_by=DOCTOR_ID,
        title="Lab Results", content="Hemoglobin is 14 g/dL."
    )
    ...
    psych_note = await create_indexed_document(
        session, patient_id=PATIENT_ALICE_ID, uploaded_by=DOCTOR_ID,
        title="Psychiatric Evaluation", content="Patient discussed anxiety symptoms."
    )
    ...
    scenarios = [
        ...
        ("3. Lab Staff: Đọc Lab Result (Truy cập hợp lệ theo document_type)", lab_staff, "Chỉ số Hemoglobin là bao nhiêu?"),
        ("4. Lab Staff: Đọc Psychiatric Note (Bị chặn do tag bypass fix)", lab_staff, "Triệu chứng lo âu thế nào?"),
        ("5. Records Staff: Đọc Clinical Note (Hợp lệ do được bổ sung role)", records_staff, "Tình trạng phẫu thuật thế nào?"),
        ("6. Doctor: Đọc Psychiatric Note (Bị chặn vì doctor chưa có scope 'psychiatry')", doctor, "Triệu chứng lo âu thế nào?"),
        ("7. Unauth User: Truy vấn Alice (Bị chặn phân quyền bệnh nhân)", unauth_user, "Tình trạng phẫu thuật thế nào?"),
        ...
        ("10. Lab Staff: Truy vấn Clinical Note chung (Hợp lệ do 'read' scope fix)", lab_staff, "Bệnh nhân có hồi phục tốt không?"),
    ]
    ...
    for desc, user, question in scenarios:
        try:
            ...
            # Actually test_chat_endpoint uses `mock_openai` fixture. I should just use `_get_conversation_history` logic or let the service run.
    ...
    with open("C:/Users/NITRO/.gemini/antigravity/brain/340ed16e-6aa9-4af3-a95c-2b034f67f732/chat_results.md", "w", encoding="utf-8") as f:
