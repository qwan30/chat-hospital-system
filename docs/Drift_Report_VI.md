...
- **Trạng thái E2E Business Logic (Playwright)**: Quá trình kiểm thử tự động với trình duyệt thực (headed mode) cho thấy toàn bộ 34/34 luồng nghiệp vụ E2E (Business Logic & Error Cases) đều đã **PASS**. Hệ thống UI Frontend hoạt động ổn định và tích hợp tốt với Backend cho các luồng nghiệp vụ chính, mặc dù phải cập nhật lại một số selector (VD: global search bar ở Topbar bị trùng với search bar ở Patients list) để bộ test có thể chạy đúng.
