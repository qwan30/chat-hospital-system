...
function Page() {
  ...
  return (
    <AppShell>
      <PageHeader title={`Request ${r.id}`} description={`${r.patient} · ${r.mrn}`} chips={<Badge variant="secondary" className="capitalize">{r.status}</Badge>} actions={<Button asChild><Link to="/access-requests/$requestId/review" params={{ requestId: r.id }}>Review</Link></Button>} />
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card className="p-5 text-sm"><h4 className="mb-2 text-sm font-semibold">Justification</h4><p className="text-muted-foreground">{r.justification}</p></Card>
      </div>
    </AppShell>
  );
}
