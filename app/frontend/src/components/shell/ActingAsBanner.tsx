...
export function ActingAsBanner() {
  ...
  return (
    <div className={cn("flex items-center justify-between gap-3 border-b px-4 py-1.5 text-xs", ROLE_TONE[session.role])}>
      ...
      <button onClick={() => setDismissed(true)} className="rounded p-0.5 hover:bg-foreground/10" aria-label="Dismiss banner">
        <X className="h-3 w-3" />
      </button>
    </div>
  );
