...
def _request() -> Request:
    return Request({"type": "http", "client": ("127.0.0.1", 8000), "method": "POST", "path": "/api/v1/documents/upload-sessions"})
...
async def test_create_upload_session_idempotency_replay(session_and_settings, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    if not doctor:
        doctor = User(id=uuid.uuid4(), email="doc@test.com", password_hash="hash", full_name="Doc", role="doctor", is_active=True)
        session.add(doctor)
...
async def test_finalize_upload_session(session_and_settings, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    doctor = await session.get(User, DOCTOR_ID)
    if not doctor:
        doctor = User(id=uuid.uuid4(), email="doc2@test.com", password_hash="hash", full_name="Doc2", role="doctor", is_active=True)
        session.add(doctor)
        await session.commit()
    ...
    def mocked_from_request(sess, req):
        ...
        service.storage = type("MockStorage", (), {
