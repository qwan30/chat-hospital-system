...
async def get_dashboard_summary(
    ...
    stmt = (
        select(
            ...
            Patient.full_name,
            Patient.mrn,
            func.max(AuditLog.created_at).label("last_accessed")
        )
    ...
    doc_stmt = (
        select(
            ...
            func.sum(case((Document.status.in_(["uploaded", "ocr_processing", "ocr_completed", "indexing"]), 1), else_=0)).label("processing"),
