...
class OcrModelManager:
    def __init__(
        ...
        *,
        ...
        self._idle_unload_generations: dict[str, int] = {}
    ...
    def _invalidate_idle_unload(self, route: str) -> int:
        generation = self._idle_unload_generations.get(route, 0) + 1
        self._idle_unload_generations[route] = generation
        return generation

    async def unload(self, route: str, *, expected_generation: Optional[int] = None) -> None:
        if expected_generation is not None and self._idle_unload_generations.get(route) != expected_generation:
            return
        self._invalidate_idle_unload(route)
    ...
    def _schedule_idle_unload(self, route: str) -> None:
        ...
        if previous is not None:
            previous.cancel()
        generation = self._invalidate_idle_unload(route)
        loop = asyncio.get_running_loop()

        def expire() -> None:
            if self._idle_unload_generations.get(route) != generation:
                return
            asyncio.create_task(self.unload(route, expected_generation=generation))
