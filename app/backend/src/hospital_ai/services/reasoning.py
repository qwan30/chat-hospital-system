...
class GraphEnhancedQAPipeline:
    """Combine vector retrieval with SQL-backed graph context.

    Steps:
    1. Run standard vector retrieval + reranking (via SimpleQA).
    2. Extract entities from the question.
    3. Query the graph for related entities and their source chunks.
    4. Augment the evidence with graph-traversed chunks.
    5. Generate a graph-aware answer.
    """
    ...
    async def run(
        ...
        *,
        ...
        simple_result = await self.simple.run(
            question=question,
            evidence=evidence,
            conversation_history=conversation_history,
        )

        # Step 2: If no DB session, fall back to simple result
        if session is None:
            return ReasoningResult(
                answer=simple_result.answer,
                citations=simple_result.citations,
                confidence=simple_result.confidence,
                disclaimer=simple_result.disclaimer,
                pipeline="graph_enhanced_qa",
            )

        # Step 3: Extract entities from the question and try graph lookup
        try:
            ...
            if query_entities:
                ...
                # If graph found additional chunks, note it in the answer
                if graph_context.related_chunk_ids:
                    graph_note = (
                        f"\n\n---\nGraph context: {graph_context.summary}"
                    )
                    return ReasoningResult(
                        answer=simple_result.answer + graph_note,
                        citations=simple_result.citations,
                        confidence=simple_result.confidence,
                        disclaimer=simple_result.disclaimer,
                        pipeline="graph_enhanced_qa",
            ...
            # Graph RAG is optional — never block the primary pipeline
            pass
