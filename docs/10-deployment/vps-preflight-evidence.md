...
> Candidate identity placeholders: `<CANDIDATE_SHA>`, `<CANDIDATE_SHORT_SHA>`, and `<CI_RUN_ID>`
...
| PENDING — operator evidence required | GHCR candidate image access | `docker manifest inspect "ghcr.io/<GHCR_NAMESPACE>/<IMAGE_NAME>:sha-<CANDIDATE_SHORT_SHA>"` | Candidate image tag or digest resolves for the operator | `<operator-recorded-value>` | `<YYYY-MM-DDThh:mm:ssZ>` | `<owner>` |
