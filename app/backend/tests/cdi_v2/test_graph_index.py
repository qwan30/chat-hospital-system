...
async def test_one_canonical_entity_keeps_independent_source_mentions(session_and_settings) -> None:
    ...
    # Setup 2 sources
    doc1 = Document(patient_id=patient_id, title="Doc 1", uploaded_by=DOCTOR_ID, mime_type="text/plain", storage_uri="mem", status="ready", document_type="clinical_note")
    doc2 = Document(patient_id=patient_id, title="Doc 2", uploaded_by=DOCTOR_ID, mime_type="text/plain", storage_uri="mem", status="ready", document_type="clinical_note")
    ...
    rev_set1 = DocumentRevisionSet(document_id=doc1.id, revision_number=1, created_by_user_id=DOCTOR_ID, status="approved", submitted_at=func.now(), approved_at=func.now(), approved_by_user_id=DOCTOR_ID)
    rev_set2 = DocumentRevisionSet(document_id=doc2.id, revision_number=1, created_by_user_id=DOCTOR_ID, status="approved", submitted_at=func.now(), approved_at=func.now(), approved_by_user_id=DOCTOR_ID)
    session.add_all([rev_set1, rev_set2])
    await session.flush()
    
    from hospital_ai.db.clinical_documents import DocumentPageRevision
    page_rev1 = DocumentPageRevision(document_id=doc1.id, page_number=1, revision_number=1, revision_type="initial", raw_text_snapshot="text", corrected_text="text", status="approved", created_by_user_id=DOCTOR_ID, content_sha256="abc")
    page_rev2 = DocumentPageRevision(document_id=doc2.id, page_number=1, revision_number=1, revision_type="initial", raw_text_snapshot="text", corrected_text="text", status="approved", created_by_user_id=DOCTOR_ID, content_sha256="abc")
    session.add_all([page_rev1, page_rev2])
    await session.flush()
    
    gen1 = DocumentIndexGeneration(document_id=doc1.id, revision_set_id=rev_set1.id, state="active", revision_set_sha256="abc", generation_sha256="abc")
    gen2 = DocumentIndexGeneration(document_id=doc2.id, revision_set_id=rev_set2.id, state="active", revision_set_sha256="def", generation_sha256="def")
    session.add_all([gen1, gen2])
    await session.flush()
    
    chunk1 = DocumentChunk(document_id=doc1.id, page_id=page1.id, patient_id=patient_id, chunk_index=0, content="text", generation_id=gen1.id, revision_set_id=rev_set1.id, page_revision_id=page_rev1.id)
    chunk2 = DocumentChunk(document_id=doc2.id, page_id=page2.id, patient_id=patient_id, chunk_index=0, content="text", generation_id=gen2.id, revision_set_id=rev_set2.id, page_revision_id=page_rev2.id)
    session.add_all([chunk1, chunk2])
    await session.flush()

    def metformin_extraction():
        return GraphExtraction(
            entities=[ExtractedEntity(entity_type="medication", normalized_label="metformin")],
            relations=[]
        )
